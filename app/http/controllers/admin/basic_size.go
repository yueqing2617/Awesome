...
func NewBasicSize() *BasicSize {
	return &BasicSize{
		//Inject services
	}
}

// Index is the index action of BasicSize controller.
func (r *BasicSize) Index(ctx http.Context) {
	name := ctx.Request().Query("name", "")
	code := ctx.Request().Query("code", "")
	page, _ := strconv.Atoi(ctx.Request().Query("page", "1"))
	limit, _ := strconv.Atoi(ctx.Request().Query("limit", "10"))
	// query
	query := facades.Orm.Query().Model(&models.BasicSize{})
	if name != "" { // 模糊
...
// Show is the show action of BasicSize controller.
func (r *BasicSize) Show(ctx http.Context) {
	...
	var data models.BasicSize
	if err := facades.Orm.Query().Model(&models.BasicSize{}).Where("id", key).OrWhere("name", key).First(&data); err != nil {
		helper.RestfulError(ctx, "查询失败："+err.Error())
		return
	}
	helper.RestfulSuccess(ctx, "查询成功", http.Json{
...
// Create is the creation action of BasicSize controller.
func (r *BasicSize) Create(ctx http.Context) {
	...
	// unique
	var count int64
	facades.Orm.Query().Model(&models.BasicSize{}).Where("name", params.Name).Count(&count)
	if count > 0 {
		helper.RestfulError(ctx, "创建失败：名称已存在")
		return
	}
	// Create your model here
	data := models.BasicSize{
		Name:   params.Name,
		Code:   params.Code,
...
// Store is the store action of BasicSize controller.
func (r *BasicSize) Store(ctx http.Context) {
	key := ctx.Request().Input("key")
	if key == "" {
		helper.RestfulError(ctx, "表单参数错误：key不能为空")
		return
	...
	if err := facades.Orm.Query().Model(&models.BasicSize{}).Where("id", data.ID).Updates(&update); err != nil {
...
// Delete is the deletion action of BasicSize controller.
func (r *BasicSize) Delete(ctx http.Context) {
	ids := ctx.Request().QueryArray("ids")
	if len(ids) == 0 {
		helper.RestfulError(ctx, "表单参数错误：ids不能为空")
		return
	}
	if err := facades.Orm.Query().Delete(&models.BasicSize{}, ids); err != nil {
