...
// Index is the index action of BasicSalesman controller.
func (r *BasicSalesman) Index(ctx http.Context) {
	...
	// query
	query := facades.Orm.Query().Model(&models.BasicSalesman{})
	if name != "" { // 模糊
...
// Show is the show action of BasicSalesman controller.
func (r *BasicSalesman) Show(ctx http.Context) {
	...
	if err := facades.Orm.Query().Model(&models.BasicSalesman{}).Where("id", key).OrWhere("name", key).First(&data); err != nil {
		helper.RestfulError(ctx, "查询失败："+err.Error())
		return
	}
	helper.RestfulSuccess(ctx, "查询成功", http.Json{
...
// Create is the creation action of BasicSalesman controller.
func (r *BasicSalesman) Create(ctx http.Context) {
	...
	// unique
	var count int64
	facades.Orm.Query().Model(&models.BasicSalesman{}).Where("name", params.Name).Count(&count)
	if count > 0 {
		helper.RestfulError(ctx, "创建失败：名称已存在")
		return
	}
	// Create your model here
	data := models.BasicSalesman{
		Name:   params.Name,
		Code:   params.Code,
...
// Store is the store action of BasicSalesman controller.
func (r *BasicSalesman) Store(ctx http.Context) {
	key := ctx.Request().Input("key")
	if key == "" {
		helper.RestfulError(ctx, "表单参数错误：key不能为空")
		return
	...
	if err := facades.Orm.Query().Model(&models.BasicSalesman{}).Where("id", data.ID).Updates(&update); err != nil {
...
// Delete is the deletion action of BasicSalesman controller.
func (r *BasicSalesman) Delete(ctx http.Context) {
	...
	if err := facades.Orm.Query().Delete(&models.BasicSalesman{}, ids); err != nil {
