...
func (r *BasicColor) Create(ctx http.Context) {
	...
	var data models.BasicColor
	err = facades.Orm.Query().Where("name", params.Name).FirstOrCreate(&data, models.BasicColor{
		Name:   params.Name,
		Code:   params.Code,
...
func (r *BasicColor) Edit(ctx http.Context) {
	var params requests.AdminBasicRequest
	valid, err := ctx.Request().ValidateRequest(&params)
	if err != nil {
		helper.RestfulError(ctx, "表单参数错误1："+err.Error())
		return
	}
	if valid != nil {
		helper.RestfulError(ctx, "表单参数错误2："+helper.GetRequestError(valid.All()))
		return
	}
